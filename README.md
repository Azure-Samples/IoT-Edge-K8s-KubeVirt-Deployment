...
3. Run `helm install --generate-name . --set azIotEdgeConfigFile=config.toml,publicSshKey="$(cat ~/.ssh/id_rsa.pub)",aziotEdgeVmEnableExternalSsh=true,aziotEdgeVmDiskSize=4Gi,nameOverride=aziot-edge-kubevirt`
